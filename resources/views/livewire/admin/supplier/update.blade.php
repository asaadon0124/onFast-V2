    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="myModalLabel110">تعديل المورد {{$supplier->name ?? '' }}  </h5>
                                 ...
                                 <select class="form-control select2" wire:model="governorate_id" wire:change="change_gov($event.target.value)">
                                    ...
                                    @if (isset($governorates))
                                        @foreach ($governorates as $governorate)
                                            <option value="{{ $governorate->id }}" @if ($governorate_id == $governorate->id)selected
                                   ...
                                   <select class="form-control select2" wire:model="city_id">
                                    ...
                                    @if (isset($governorates))
                                        @foreach ($cities as $city)
                                            <option value="{{ $city->id }}" @if ($city_id == $city->id)selected
